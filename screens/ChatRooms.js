        ...
        return (
            <View>
                <TouchableOpacity style={styles.container} onPress={() => navigation.navigate('Message',
                    { userUid: item.sender._id })}>
                    <View style={{ flexDirection: 'row' }}>
                        <Avatar source={{ uri: userAvatar && userAvatar }} rounded size='large' />
                        <View style={{ marginLeft: 50 }}>
                            <Text style={[styles.title]}>{item.latestMessage.user.name}</Text>
                            <Text style={styles.subtitle}><Text style={{ color: 'black' }}>son mesaj: </Text>{item.latestMessage.text}</Text>
    ...
    if (emptyArray) {
        return (
            <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
                <Text style={{ fontFamily: 'Poppins-Regular', fontSize: 18 }}>Henüz mesajın yok.</Text>
            </View>
        )
    }
    return (
