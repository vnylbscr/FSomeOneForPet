...
import {Avatar, Overlay, Input, Button, ListItem} from 'react-native-elements';
...
const BG_IMAGE =
  'https://imgix.bustle.com/uploads/shutterstock/2020/10/15/2a026e04-dd04-445c-b73f-695db7cd1183-shutterstock-1444203920.jpg?w=1020&h=574&fit=crop&crop=faces&auto=format%2Ccompress';
const ShowProfile = ({route, navigation}) => {
  ...
  const showCommentsModal = () => {
    return (
      <Overlay
        onBackdropPress={() => setShowCommentsVisible(!showCommentsVisible)}
        overlayStyle={styles.showCommentsModalContainer}
        isVisible={showCommentsVisible}>
        <ScrollView>
          <Text style={styles.modalText}>
            {userFromDb && userFromDb.username} adlı kullanıcının
            değerlendirmeleri
          </Text>
          {comments &&
            comments.length > 0 &&
    ...
    return (
      userFromDb && (
        <Overlay
          ...
          <Text style={styles.modalText}>
            {userFromDb.username} adlı kullanıcıya değerlendirme ekle.{' '}
          </Text>
          <Input
          ...
          <ImageBackground
            ...
            <View style={{alignItems: 'center'}}>
              ...
              <Text style={styles.textStyle}>
                {userFromDb && userFromDb.username} ({' '}
                {userFromDb && userFromDb.age} )
              </Text>
              <Text style={styles.textSubTitle}>Host, Surfer</Text>
              <Text style={styles.textSubTitle}>
                {userFromDb && userFromDb.about}
              </Text>
              {userFromDb && <Text>{userFromDb.job}</Text>}
              <Text style={styles.textSubTitle}>
                {userFromDb && userFromDb.location}
              </Text>
            </View>
          </ImageBackground>
        </View>
        ...
        <View style={styles.userInfo}>
          <View style={{flex: 2}}>
            <View style={{flexDirection: 'row', marginTop: 5}}>
              ...
              <Text style={styles.infoTextIcon}>
                {userFromDb && userFromDb.location
                  ? userFromDb.location
                  : 'Konum Belirtilmedi'}
              </Text>
          ...
          <View style={{flex: 2}}>
            ...
            {userFromDb &&
              userFromDb.availablePets.length > 0 &&
              userFromDb.availablePets.map((item, index) => {
                return (
                  <View style={{flexDirection: 'column'}}>
                    <View style={{flexDirection: 'row'}} key={index}>
                      <Icon name='paw-outline' size={25} color='#f7a440' />
                      <Text key={index} style={styles.infoTextIcon}>
                        {item}
                      </Text>
                    </View>
                  </View>
